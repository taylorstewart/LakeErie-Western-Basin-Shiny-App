...
catch <- read.csv("data/WB_Catch.csv",header=T)%>%filter(species!="Uid Minnows")
lw <- read.csv("data/WB_LengthWeight.csv",header=T)%>%filter(species!="Uid Minnows")
...
## -----------------------------------------------------------
wb_wq%<>%mutate(serial=as.numeric(serial),year=as.numeric(year),season=as.character(season),serial=ifelse(serial>200,serial-500,serial-100))%>%filter(!is.na(serial),!(year==2015&season=="Autumn"),!(year==2016&season=="Autumn"))%>%
  select(year,season,serial,bin.depth.max,temp.mean,do.ppm.mean)
