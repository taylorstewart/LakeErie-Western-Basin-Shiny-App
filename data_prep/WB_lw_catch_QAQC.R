...
### Read in data
trawl <- loadWorkbook("/Volumes/STEWART/WBfall2015_trawl.xlsx")
lw <- readWorksheet(trawl,sheet="LW data") %>% 
  select(serial=serial.,species,size,tl,wt) %>% 
  slice(-1)
catch <- readWorksheet(trawl,sheet="catch data") %>% 
  select(serial,species,size,count_final) %>% 
  slice(-1)
